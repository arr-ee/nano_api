describe("sorting", function(){
  ...
  var minimal_price = null;
  var maximum_price = null;
  ...
  beforeEach(function(){
    ...
    NANO.searchResults.create(copied_search_json, function (results){
      ...
      maximum_price = _.max(search_results.tickets.get(), function(ticket){
        return ticket.minimal_price();
      });
    });
  });
  ...
  it("price sorting asc", function(){
    waitsFor(function(){
      return search_results !== null;
    });
    runs(function(){
      var sorted_tickets = search_results.tickets.sort("price", "acs");
      var price = sorted_tickets.get()[0].minimal_price();
      expect(price).toBe(minimal_price);
    });
  });

  it("price sorting desc", function(){
    waitsFor(function(){
      return search_results !== null;
    });
    runs(function(){
      var sorted_tickets = search_results.tickets.sort("price", "desc");
      var price = sorted_tickets.get()[0].minimal_price();
      expect(price).toBe(maximum_price);
    });
  });

  it("double price sorting", function(){
    waitsFor(function(){
      return search_results !== null;
    });
      runs(function(){
      var sorted_tickets = search_results.tickets.sort("price", "desc").sort("price", "desc");
      var price = sorted_tickets.get()[0].minimal_price();
      expect(price).toBe(maximum_price);
    });
  });
