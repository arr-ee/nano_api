NANO("api.search.filters", function(NANO){
  NANO.search.customize("each", "tickets", function(ticket_proto){
    ticket_proto.current_info = function(){
      ...
      if(filter && filter.gates){
        ...
        _.each(this.gates(), function(gate){
          var is_payment_methods_fit = true;
          if(filter.payment_methods){
            is_payment_methods_fit = !!_.find(gate.payment_methods, function(method){
              return filter.payment_methods[method];
            });
          }
          if(gates_filter[gate.id] && is_payment_methods_fit){
            gates.push(gate);
          }
        });
      } else {
    ...
    ticket_proto.current_payment_methods = function(){
      var payment_methods = [];
      _.each(this.current_info().gates, function(gate){
        payment_methods = payment_methods.concat(gate.payment_methods);
      });
    ...
    ticket_proto.filter_data = function(){
      ...
      var get_flights_data = function(func_field){
        ...
        return _.without(_.flatten(results), undefined, "");
      };
      var filter_data = {
        direct_flight_time: direct_flights.depart_time(),
        return_flight_time: return_flights ? return_flights.depart_time() : null,
        flights_duration: get_flights_data("duration"),
        stops_airports: get_flights_data("stops_iatas"),
        stops_count: [_.max(get_flights_data("stops_count"))],
        ...
        payment_methods: this.current_payment_methods()
      };
  ...
  NANO.search.customize("collection", "tickets", function(proto){
    var utils = {
      ...
      is_includes_range: function(range, edges){
        return utils.is_in_range(range, _.min(edges)) && utils.is_in_range(range, _.max(edges));
      }
    };
    var filter = {
      ...
      matchers: {
        ...
        stops_count: function(filter_params, ticket_params){
          return utils.strict_check_list_param(filter_params, ticket_params);
        },
        ...
        payment_methods: utils.check_list_param,
    ...
    var string_fields = ['origin', 'destination', 'stops_airports', "alliances",
      'airlines', 'alliances', 'gates', 'stops_count', 'payment_methods'];
