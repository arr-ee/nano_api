//= require utils/levenstain
//= require api/dictionary
//= require api/routes

NANO("api.autocomplete", function(NANO){
  var dictionary = null;
  ...
  Source.prototype = {
    ...
    _get_from_dictionary: function(term, handler){
      ...
      if(!results) {
        return false;
      } else if(this.options.items_count < results.length){
        handler(results, term);
        return true;
      } else if(0 < results.length < this.options.items_count){
        handler(results, term);
        return false;
      }
    },
    ...
    slice_and_sort: function(matches, term){
      ...
      return _.sortBy(matches, function(match_item){
        ...
        var distance = NANO.utils.levenstain.calculate(term, match_item.name);
        return short_term ? 
          Math.min(NANO.utils.levenstain.calculate(term, match_item.iata), distance) : 
          distance;
  ...
  autocomplete.create(NANO.api.dictionary.create("places", window.AutocompletePlaces || []), "places");
  return autocomplete;
}, {
  require: ["api.dictionary", "utils.levenstain", "api.routes"]
});