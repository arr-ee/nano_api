NANO.plugin(function(NANO){
  ...
  var Source = function(dictionary, max_items){
    this.dict = dictionary;
    this.max_items_count = max_items || 10;
    return _.bind(this.get, this);
  };
  Source.prototype = {
    ajax_data: {
      url: '/places_' + (window.LOCALE || "ru"),
      dataType: 'json'
    },
    ...
    _get_from_dictionary: function(term, handler){
      ...
      if(this._is_iata(term)){
        results = this.dict.iata_search(term);
        results = results !== null ? [results] : null;
      }else {
        results = this.dict.soft_search(term);
      }
    ...
    _get_form_ajax: function(term, handler){
      ...
      this._abort_request();
      var self = this;
      this.current_ajax_requrest = $.ajax($.extend(this.ajax_data, {
        data: {term: term},
    ...
    slice_and_sort: function(matches, term){
      ...
      return _.sortBy(matches, function(match_item){
        ...
        return short_term ? 
          ...
          distance;
      }).slice(0, this.max_items_count);
    }
  };
     
  NANO.autocomplete = {
    create: function(dictionary, name){
      this[name] = {
        dictionary: dictionary,
        get: new Source(dictionary)
      };
    }
  };
